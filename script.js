...
function ultimos4(doc){
 const d=clean(doc);
 if(d.length >= 6) return d.slice(-6, -2); // para 00.000.000/4801-00 retorna 4801
 return d;
}
function codigoCliente(c){return c.codigo || ultimos4(c.doc)}
...
function parseNumberBR(v){return Number(String(v||"").replace(/\D/g,"")||0)}
function calcularCompras(pontos, comprasDigitadas){
 const c=parseNumberBR(comprasDigitadas);
 return c>0 ? c : Number(pontos||0);
}

function init(){
 if(!localStorage.getItem("dc_clientes")){
  localStorage.setItem("dc_clientes",JSON.stringify([
   {nome:"ACONCHEGO ARQUITETURA",doc:"00.000.000/7272-00",codigo:"7272",pontos:1146,compras:1146,categoria:"PORCELANATO"},
   {nome:"ADALBERTO NETO",doc:"00.000.000/6919-00",codigo:"6919",pontos:8175,compras:8175,categoria:"PORCELANATO"}
  ]));
 }
 ...
 if(!localStorage.getItem("dc_resgates")) localStorage.setItem("dc_resgates",JSON.stringify([]));
 renderAll();
}
...
function buscarPorTermo(termo){
 ...
 const termoNum=clean(raw);
 const termoTxt=normalizarTexto(raw);
 const lista=clientes();

 if(!raw) return null;

 return lista.find(c=>{
  const doc=clean(c.doc);
  const cod=String(codigoCliente(c)||"");
  const nome=normalizarTexto(c.nome);
  return (
   doc === termoNum ||
   cod === termoNum ||
   doc.includes(termoNum) ||
   termoNum.includes(doc) ||
   (termoNum.length >= 3 && cod.includes(termoNum)) ||
   (termoTxt.length >= 3 && nome.includes(termoTxt))
  );
 });
}

function consultarCliente(){
 const c=buscarPorTermo(document.getElementById("clienteDoc").value);
 const box=document.getElementById("clienteBox");
 if(!c){
  box.innerHTML="<hr><b>Cliente não encontrado.</b><p>Digite apenas o código curto, CNPJ completo, CNPJ sem pontuação ou nome. Exemplo: 7272.</p>";
  return;
 }
 ...
 box.innerHTML=`
  <hr>
  <h3>Olá, ${c.nome}</h3>
  <p><b>Código:</b> ${codigoCliente(c)}</p>
  <p><b>Documento:</b> ${c.doc}</p>
  <p><b>Pontos:</b> ${pts(c.pontos)}</p>
  <p><b>Compras acumuladas:</b> ${money(c.compras)}</p>
  <p><b>Categoria:</b> ${c.categoria||"Produtos Black"}</p>
...
function prepararResgate(doc){
 showPage("resgate");
 document.getElementById("resgateDoc").value=codigoCliente({doc});
 consultarParaResgate();
}

function consultarParaResgate(){
 const c=buscarPorTermo(document.getElementById("resgateDoc").value);
 const box=document.getElementById("resgateClienteBox");
 clienteResgateAtual=c||null;
 if(!c){
  box.innerHTML='<div class="alert-error"><b>Cliente não encontrado.</b><br>Confira o CPF/CNPJ, código ou nome.</div>';
  return;
 }
 box.innerHTML=`<div class="alert-ok"><b>${c.nome}</b><br>Código: ${codigoCliente(c)}<br>Documento: ${c.doc}<br>Saldo: ${pts(c.pontos)}<br>Compras: ${money(c.compras)}</div>`;
}

function solicitarResgate(){
 const msg=document.getElementById("resgateMsg");
 if(!clienteResgateAtual){consultarParaResgate();}
 ...
 const arr=resgates();
 arr.push({data:new Date().toLocaleString("pt-BR"),nome:clienteResgateAtual.nome,doc:clienteResgateAtual.doc,codigo:codigoCliente(clienteResgateAtual),premio,pontos:custoNum,whatsapp:document.getElementById("resgateWhatsapp").value.trim(),obs:document.getElementById("resgateObs").value.trim(),status:"Pendente"});
...
function salvarCliente(){
 ...
 const novo={nome:document.getElementById("nomeCliente").value.trim(),doc:doc,codigo:ultimos4(doc),pontos:pontos,compras:calcularCompras(pontos,document.getElementById("comprasCliente").value),categoria:"MANUAL"};
 if(!novo.nome||!clean(novo.doc)){alert("Preencha nome e documento/código.");return;}
 const i=arr.findIndex(x=>clean(x.doc)===clean(novo.doc) || codigoCliente(x)===novo.codigo);
...
function salvarPromo(){
 ...
 arr.push(nova); localStorage.setItem("dc_promos",JSON.stringify(arr)); renderAll(); alert("Promoção salva!");
}
...
function separarCSVLinha(linha){
 const sep=linha.includes(";")?";":",";
 return linha.split(sep).map(x=>(x||"").trim());
}
...
function importarCSV(){
 ...
 linhas.forEach(l=>{
  const dados=separarCSVLinha(l);
  let nome=(dados[0]||"").trim();
  let doc=(dados[1]||"").trim();
  let pontos=parseNumberBR(dados[2]||"0");
  let categoria=(dados[3]||"PORCELANATO").trim();
  if(nome.toLowerCase()==="nome") return;
  if(!nome||!doc||!pontos){ignorados++; return;}
  const novo={nome,doc,codigo:ultimos4(doc),pontos,compras:pontos,categoria};
  const i=arr.findIndex(x=>clean(x.doc)===clean(novo.doc) || codigoCliente(x)===novo.codigo);
  if(i>=0) arr[i]=novo; else arr.push(novo);
  importados++;
 });
...
function limparClientes(){
 if(confirm("Deseja apagar todos os clientes cadastrados neste navegador?")){
  localStorage.setItem("dc_clientes",JSON.stringify([])); renderAll(); alert("Clientes apagados.");
 }
}
function atualizarStatusResgate(index,status){
 const arr=resgates(); if(arr[index]){arr[index].status=status;localStorage.setItem("dc_resgates",JSON.stringify(arr));renderAll();}
}
function excluirResgate(index){
 if(confirm("Excluir solicitação de resgate?")){const arr=resgates(); arr.splice(index,1); localStorage.setItem("dc_resgates",JSON.stringify(arr)); renderAll();}
}
function baixarModeloCSV(){
 const conteudo="nome,documento,pontos,categoria\nACONCHEGO ARQUITETURA,00.000.000/7272-00,1146,PORCELANATO\nADALBERTO NETO,00.000.000/6919-00,8175,PORCELANATO\n";
 const blob=new Blob([conteudo],{type:"text/csv;charset=utf-8"});
 const url=URL.createObjectURL(blob); const a=document.createElement("a");
 a.href=url; a.download="modelo_importacao_dellapiani.csv"; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
}
...
function renderAll(){
 ...
 if(productGrid){
  productGrid.innerHTML=produtosBlack.map(p=>`
   <article class="product-card"><img src="${p.img}" alt="${p.nome}" onerror="this.style.display='none';"><div class="product-info"><h3>${p.nome}</h3><p>${p.desc}</p><div class="product-tags">${p.tags.map(t=>`<span>${t}</span>`).join("")}</div><button class="btn gold" onclick="showPage('cliente')">Consultar pontos</button></div></article>
  `).join("");
 }
 ...
 document.getElementById("rankingBody").innerHTML=ordenados.map((c,i)=>`<tr><td>#${i+1}</td><td>${c.nome}</td><td>${c.doc}</td><td>${pts(c.pontos)}</td><td>${money(c.compras)}</td><td>${c.pontos>=100000?"Premium":"Ativo"}</td></tr>`).join("");
 document.getElementById("adminClientes").innerHTML=cs.map(c=>`<tr><td>${c.nome}</td><td>${c.doc}</td><td>${codigoCliente(c)}</td><td>${pts(c.pontos)}</td><td><button onclick="excluirCliente('${c.doc}')" class="btn danger">Excluir</button></td></tr>`).join("");
 document.getElementById("adminResgates").innerHTML=rs.map((r,i)=>`<tr><td>${r.data}</td><td>${r.nome}</td><td>${r.codigo||""}</td><td>${r.premio}</td><td>${pts(r.pontos)}</td><td>${r.whatsapp||"-"}</td><td>${r.status}</td><td><button onclick="atualizarStatusResgate(${i},'Aprovado')" class="btn ok">Aprovar</button><button onclick="atualizarStatusResgate(${i},'Negado')" class="btn danger">Negar</button><button onclick="excluirResgate(${i})" class="btn dark">Excluir</button></td></tr>`).join("");
